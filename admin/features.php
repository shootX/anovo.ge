<?php
session_start();
if (!isset($_SESSION['admin_user'])) {
    header('Location: login.php');
    exit;
}

$mysqli = new mysqli("localhost", "admin_anovoroot", "Dr@nda252", "admin_anovo");
$mysqli->set_charset("utf8mb4");

// შენახვა
$success = false;
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // წავშალოთ ძველი
    $mysqli->query("DELETE FROM content WHERE section='features'");
    $titles = $_POST['title'] ?? [];
    $texts = $_POST['text'] ?? [];
    foreach ($titles as $i => $title) {
        $title = $mysqli->real_escape_string($title);
        $text = $mysqli->real_escape_string($texts[$i] ?? '');
        $mysqli->query("INSERT INTO content (section, field, value) VALUES ('features', '{$i}_title', '$title')");
        $mysqli->query("INSERT INTO content (section, field, value) VALUES ('features', '{$i}_text', '$text')");
    }
    $success = true;
}

// წაკითხვა
$features = [];
$res = $mysqli->query("SELECT field, value FROM content WHERE section='features'");
foreach ($res as $row) {
    if (preg_match('/^(\d+)_(title|text)$/', $row['field'], $m)) {
        $idx = (int)$m[1];
        $type = $m[2];
        if (!isset($features[$idx])) $features[$idx] = ['title'=>'','text'=>''];
        $features[$idx][$type] = $row['value'];
    }
}
ksort($features);
?>
<!DOCTYPE html>
<html lang="ka">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>Features მართვა – Admin</title>
  <link href="bootstrap.min.css" rel="stylesheet">
  <style>
    body { background: #0f172a; color: #fff; font-family: Inter, sans-serif; }
    .card { background: rgba(255,255,255,0.04); border-radius: 1.5rem; }
    label { color: #fff; }
    .remove-btn { cursor:pointer; color:#ef4444; font-size:1.2em; }
  </style>
</head>
<body>
  <header class="bg-black text-white py-4 px-6 md:px-12 d-flex justify-content-between align-items-center">
    <div class="flex items-center">
      <span class="text-xl font-bold"><span class="text-danger">kogan.</span>design Admin</span>
    </div>
    <div>
      <a href="dashboard.php" class="btn btn-outline-light btn-sm me-2">დასაბრუნებლად</a>
      <a href="logout.php" class="btn btn-outline-light btn-sm">Logout</a>
    </div>
  </header>
  <main class="container my-5">
    <div class="card p-4 mb-5 mx-auto" style="max-width:700px;">
      <h1 class="text-2xl fw-bold mb-4 text-danger">Features მართვა</h1>
      <?php if($success): ?><div class="alert alert-success">მონაცემები შენახულია</div><?php endif; ?>
      <form method="post" autocomplete="off" id="features-form">
        <div id="features-list">
          <?php foreach($features as $i => $f): ?>
          <div class="row mb-3 feature-row align-items-end">
            <div class="col-md-5">
              <label class="form-label">სათაური</label>
              <input type="text" name="title[]" class="form-control form-control-lg" value="<?php echo htmlspecialchars($f['title']); ?>" required>
            </div>
            <div class="col-md-6">
              <label class="form-label">ტექსტი</label>
              <input type="text" name="text[]" class="form-control form-control-lg" value="<?php echo htmlspecialchars($f['text']); ?>" required>
            </div>
            <div class="col-md-1 text-end">
              <span class="remove-btn" onclick="removeFeature(this)">&times;</span>
            </div>
          </div>
          <?php endforeach; ?>
        </div>
        <button type="button" class="btn btn-secondary mb-3" onclick="addFeature()">+ დამატება</button>
        <button type="submit" class="btn btn-danger w-100">შენახვა</button>
      </form>
    </div>
  </main>
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
  <script>
    function addFeature() {
      const html = `<div class=\"row mb-3 feature-row align-items-end\">\n  <div class=\"col-md-5\">\n    <label class=\"form-label\">სათაური<\/label>\n    <input type=\"text\" name=\"title[]\" class=\"form-control form-control-lg\" required>\n  <\/div>\n  <div class=\"col-md-6\">\n    <label class=\"form-label\">ტექსტი<\/label>\n    <input type=\"text\" name=\"text[]\" class=\"form-control form-control-lg\" required>\n  <\/div>\n  <div class=\"col-md-1 text-end\">\n    <span class=\"remove-btn\" onclick=\"removeFeature(this)\">&times;<\/span>\n  <\/div>\n<\/div>`;
      document.getElementById('features-list').insertAdjacentHTML('beforeend', html);
    }
    function removeFeature(el) {
      el.closest('.feature-row').remove();
    }
  </script>
</body>
</html> 